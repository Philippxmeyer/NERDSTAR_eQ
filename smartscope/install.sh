...
install -d -m 755 -o "$PI_USER" -g "$PI_USER" "$APP_DIR/data"
...
CATALOG="$APP_DIR/data/catalog.xml"
if [[ -f "$CATALOG" ]]; then
    ENTRY_COUNT=$(grep -c '<object' "$CATALOG" 2>/dev/null || echo "?")
    info "Catalog installed: $CATALOG ($ENTRY_COUNT objects)"
else
